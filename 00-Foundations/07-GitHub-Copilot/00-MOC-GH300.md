...
**Trạng thái:** ✅ **HOÀN TẤT 18/18 note** (cụm A → F) + **22 ảnh** minh hoạ.

> 📋 **Bảng trong nguồn:** 2 file docx có **14 bảng** (đã bị flatten thành đoạn tab-separated khi copy, `len(doc.tables) == 0`), **tất cả đã được dựng lại thành bảng Markdown** trong các note tương ứng.
...
| # | Note | Nội dung | Trạng thái |
|---|------|----------|------------|
| 7 | [[07-Copilot-trong-IDE\|Copilot trong IDE]] | Completion, multiple suggestions pane, `#`/`@`/`/`, 7 slash command, PRU theo model | ✅ |
| 8 | [[08-Copilot-tren-GitHub-com\|Copilot trên GitHub.com]] | 5 nhóm agent task chạy nền, PR summary 1-2 PRU, review 1-3 PRU, Explain error in Actions | ✅ |
| 9 | [[09-Copilot-CLI-va-GitHub-Copilot-App\|Copilot CLI & Copilot app]] | Slash command CLI, local vs cloud sandbox, Git worktree, 3 session mode, `/chronicle` | ✅ |

## Cụm D — Agent & MCP

| # | Note | Nội dung | Trạng thái |
|---|------|----------|------------|
| 10 | [[10-Agent-Mode-trong-IDE\|Agent mode trong IDE]] | 4 mức tương tác, chu kỳ 4 bước, draft→review→accept, self-healing, PRU, limitations | ✅ |
| 11 | [[11-Copilot-Cloud-Agent\|Copilot Cloud Agent]] | Giao issue (UI + GraphQL), 6 lớp bảo mật, nhánh `copilot/`, giới hạn, `copilot-setup-steps.yml`, MCP mặc định | ✅ |
| 12 | [[12-GitHub-MCP-Server\|GitHub MCP Server]] | MCP "USB-C của AI", 3 kiểu kết nối, OAuth/PAT/Docker, agentic loop, 5 best practice | ✅ |

## Cụm E — Review, quản trị, năng suất, kiểm thử

| # | Note | Nội dung | Trạng thái |
|---|------|----------|------------|
| 13 | [[13-Code-Review-va-Pull-Request\|Code review & PR]] | 5 tính năng, comment review không chặn merge, `applyTo`, Rulesets 3 mức, tối ưu PRU | ✅ |
| 14 | [[14-Quan-tri-va-Tuy-bien\|Quản trị & tuỳ biến]] | 2 bảng tính năng theo gói, IP indemnity cần Block, 3 scope filter, content exclusions + 3 giới hạn | ✅ |
| 15 | [[15-Developer-Use-Cases-va-Do-luong\|Use cases & đo lường]] | 4 nhóm use case, 5 giai đoạn SDLC, 4 agent orchestration, REST API metrics, survey | ✅ |
| 16 | [[16-Unit-Testing-voi-Copilot\|Unit testing với Copilot]] | C# Dev Kit + 3 framework, Ask/Plan/Agent, `/setupTests` `/tests` `/fixTestFailure`, ghost text | ✅ |

## Cụm F — Thực hành & ôn thi

| # | Note | Nội dung | Trạng thái |
|---|------|----------|------------|
| 17 | [[17-Thuc-hanh-Copilot-theo-Ngon-ngu\|Thực hành theo ngôn ngữ]] | Codespaces, ghost text, implicit prompt, selective context, luồng JS & Python | ✅ |
| 18 | [[18-GH-300-Cheatsheet-va-QA\|⭐ Cheatsheet + Q&A]] | 10 bảng tra, **22 cặp dễ nhầm**, bảng PRU tổng hợp, bản đồ 7 domain, 18 Q&A | ✅ |

> 🎯 **Ôn gấp trước giờ thi?** Đọc thẳng [[18-GH-300-Cheatsheet-va-QA]] — gom toàn bộ con số, định danh và bảng phân biệt của 17 note còn lại.
